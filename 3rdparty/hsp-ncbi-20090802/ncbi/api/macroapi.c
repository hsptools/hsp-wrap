  ...
  switch (field->choice) {
    ...
    case FieldType_feature_field:
      ...
      if (feature_field != NULL && feature_field->field != NULL
          ...
          && (feature_field->field->data.intvalue == Feat_qual_legal_db_xref || feature_field->field->data.intvalue == Feat_qual_legal_ec_number)) {
        rval = TRUE;
      }
      break;
    case FieldType_cds_gene_prot:
...
static void AddStructuredCommentDescriptorDestinationsForBioseq (BioseqPtr bsp, ValNodePtr PNTR dest_list)
{
  SeqDescrPtr sdp;
  SeqMgrDescContext context;
  Boolean found = FALSE;
...
static ValNodePtr CollectStructuredCommentsForApply (SeqEntryPtr sep, ValNodePtr constraint)
{
  ...
  for (vnp = tmp_list; vnp != NULL; vnp = vnp->next) {
    AddStructuredCommentDescriptorDestinationsForBioseq (vnp->data.ptrvalue, &target_list);
  }
  tmp_list = ValNodeFree (tmp_list);
  return target_list;
}
...
static Boolean ApplyFixCapsToSeqEntry (SeqEntryPtr sep, FixCapsActionPtr action, FILE *log_fp)
{
  ...
  switch (action->choice) {
    case FixCapsAction_pub:
      num = ApplyFixPubCapsToSeqEntry (action->data.ptrvalue, sep);
      if (num > 0) {
        if (log_fp != NULL) {
...
static void ApplyFixPubCapsCallback (PubdescPtr pdp, Pointer data)
{
  ...
  if (f->action->affiliation) {
    ...
    if (alp != NULL && alp->affil != NULL && !StringHasNoText (alp->affil->country)) {
      ...
      if (StringCmp (alp->affil->country, "USA") == 0 && !StringHasNoText (alp->affil->sub)) {
...
static void FindDuplicateFeatsCallback (BioseqPtr bsp, Pointer data)
{
  ...
  while (sfp1 != NULL) {
    ...
    if (DoFeaturesMatch (sfp1, sfp2, FALSE, dfp->action->case_sensitive, dfp->action->ignore_partials)) {
      ValNodeAddPointer (&(dfp->delete_list), OBJ_SEQFEAT, sfp2);
    }
    sfp1 = sfp2;
  }
...
const CharPtr s_SpecialPubFieldWords [] = { "is present", "is not present", "is all caps", "is all lowercase" };
