...

{
  ...
  /* compare indicated country/province to guess/promitate country/water */
  ...
  if (adjust) {
    ...
    if ((test & eLatLonClassify_CountryMatch) && (test & eLatLonClassify_ProvinceMatch)) {
      ...
      if (llm.claimedfull != NULL) {
        fmt = "Lat_lon '%s' maps to '%s' instead of '%s' - claimed region '%s' is at distance %d km";
        if (province != NULL) {
          if (StringICmp (llm.guesscountry, country) == 0) {
            if (vsp->testLatLonSubregion) {
              ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState, fmt, lat_lon, llm.fullguess, fullname, llm.claimedfull, llm.claimeddistance);
            }
          } else {
            ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonCountry, fmt, lat_lon, llm.fullguess, fullname, llm.claimedfull, llm.claimeddistance);
          }
        } else {
          ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonCountry, fmt, lat_lon, llm.fullguess, country, llm.claimedfull, llm.claimeddistance);
        }
      } else {
        fmt = "Lat_lon '%s' maps to '%s' instead of '%s'";
        if (StringICmp (llm.guesscountry, country) == 0) {
          if (vsp->testLatLonSubregion) {
            ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState, fmt, lat_lon, llm.fullguess, fullname);
          }
        } else {
          ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonCountry, fmt, lat_lon, llm.fullguess, fullname);
        }
      }
...
static void PrevValidateCountryLatLon (
  ValidStructPtr vsp,
  GatherContextPtr gcp,
  CharPtr countryname,
  CharPtr lat_lon
...
{
  CharPtr     bodyofwater = NULL;
  CtrySetPtr  csp;
  Char        ctry [256];
  FloatHi     distance, nautical;
  Boolean     format_ok = FALSE, lat_in_range = FALSE, lon_in_range = FALSE;
  CharPtr     guess = NULL;
  FloatHi     lat = 0.0;
  FloatHi     lon = 0.0;
  ...
  if (vsp == NULL || gcp == NULL || countryname == NULL || lat_lon == NULL) return;
  ...
  if (format_ok && ParseLatLon (lat_lon, &lat, &lon)) {
    StringNCpy_0 (region, countryname, sizeof (region));
    ptr = StringChr (region, ',');
    if (ptr != NULL) {
      *ptr = '\0';
    }
    ptr = StringChr (region, ';');
    ...
    StringNCpy_0 (ctry, countryname, sizeof (ctry));
    ptr = StringChr (ctry, ':');
    if (ptr != NULL) {
      *ptr = '\0';
      strict = FALSE;
    ...
    if (CountryIsInLatLonList (ctry)) {
      csp = GetLatLonCountryData ();
      if (csp != NULL) {
        scale = csp->scale;
      }
      if (CountryContainsLatLon (ctry, lat, lon)) {
        /* match */
        if (! strict) {
          if (CountryIsInLatLonList (region)) {
            if (CountryContainsLatLon (region, lat, lon)) {
              /* match */
            } else {
              if (vsp->strictLatLonCountry || (vsp->testLatLonSubregion && (! StringContainsBodyOfWater (countryname)))) {
                ...
                if (StringDoesHaveText (guess)) {
                  if (StringICmp (guess, region) != 0) {
                    if (CountryExtremesOverlap (guess, region)) {
                      if (vsp->indexerVersion) {
                        ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonAdjacent,
                                  ...
                                  lat_lon, guess, region);
                      } else {
                        ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                                  "Lat_lon '%s' MIGHT be in '%s' instead of '%s'", lat_lon, guess, region);
                      }
                    } else if (CountryIsClosestToLatLon (region, lat, lon, 1.0, &distance)) {
                      ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                                 "Lat_lon '%s' is closest to designated subregion '%s' at distance %d km", lat_lon, region, AdjustAndRoundDistance (distance, scale));
                    } else {
                      ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                                 "Lat_lon '%s' might be in '%s' instead of '%s'", lat_lon, guess, region);
                    }
                    ...
                    ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                              "Lat_lon '%s' does not map to subregion '%s', but may be in '%s'", lat_lon, region, guess);
                  }
                } else {
                  if (CountryIsNearLatLon (region, lat, lon, 1.0, &distance)) {
                    ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                               "Lat_lon '%s' is nearby but not inside subregion '%s' at distance %d km", lat_lon, region, AdjustAndRoundDistance (distance, scale));
                  } else {
                    ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                              "Lat_lon '%s' does not map to subregion '%s'", lat_lon, region);
                  }
                }
              }
            }
          } else {
            /* country matches, unrecognized subregion */
          }
        } else {
          /* strict match */
        }
      } else if (CountryIsClosestToLatLon (region, lat, lon, 1.0, &distance)) {
        bodyofwater = LookupWaterByLatLon (lat, lon);
        if (StringDoesHaveText (bodyofwater)) {
          ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonWater,
                    "Lat_lon '%s' is closest to designated subregion '%s' at distance %d km, but in water '%s'",
                    lat_lon, region, AdjustAndRoundDistance (distance, scale), bodyofwater);
        } else {
          if (!vsp->indexerVersion) {
            bodyofwater = WaterClosestToLatLon (lat, lon, 1.0, &nautical);
            if (StringDoesHaveText (bodyofwater)) {
              ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonOffshore,
                        "Lat_lon '%s' is offshore of designated subregion '%s' at distance %d km, nearest water '%s' is at distance %d km",
                        lat_lon, region, AdjustAndRoundDistance (distance, scale), bodyofwater, AdjustAndRoundDistance (nautical, scale));
            } else {
              ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonOffshore,
                        "Lat_lon '%s' is offshore of designated subregion '%s' at distance %d km",
                        lat_lon, region, AdjustAndRoundDistance (distance, scale));
            }
          }
        }
      } else if (StringICmp (region, ctry) != 0 && CountryIsClosestToLatLon (ctry, lat, lon, 1.0, &distance)) {
        bodyofwater = LookupWaterByLatLon (lat, lon);
        if (StringDoesHaveText (bodyofwater)) {
          ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonWater,
                    "Lat_lon '%s' is closest to designated country '%s' at distance %d km, but in water '%s'",
                    lat_lon, ctry, AdjustAndRoundDistance (distance, scale), bodyofwater);
        } else {
          ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                    "Lat_lon '%s' is closest to designated country '%s' at distance %d km",
                    lat_lon, ctry, AdjustAndRoundDistance (distance, scale));
        }
      } else if (StringCmp (ctry, "Antarctica") == 0 && lat < -60.0) {
        /* ignore Antarctica below 60 degrees S */
      } else if (LatLonInRange (-lat, lon) && CountryContainsLatLon (ctry, -lat, lon)) {
        if (lat < 0.0) {
          ValidErr (vsp, SEV_WARNING, ERR_SEQ_DESCR_LatLonValue, "Latitude should be set to N (northern hemisphere)");
        } else {
          ValidErr (vsp, SEV_WARNING, ERR_SEQ_DESCR_LatLonValue, "Latitude should be set to S (southern hemisphere)");
      ...
      } else if (LatLonInRange (lon, lat) && CountryContainsLatLon (ctry, lon, lat)) {
        ValidErr (vsp, SEV_WARNING, ERR_SEQ_DESCR_LatLonValue, "Latitude and longitude values appear to be exchanged");
      } else {
        if (! StringContainsBodyOfWater (countryname)) {
          guess = LookupCountryByLatLon (lat, lon);
          if (StringDoesHaveText (guess)) {
            if (StringICmp (guess, ctry) != 0) {
              if (CountryExtremesOverlap (guess, ctry)) {
                if (vsp->indexerVersion) {
                  ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonAdjacent,
                            "Lat_lon '%s' MIGHT be in '%s' instead of adjacent '%s' - SHIFT DOUBLE CLICK TO LAUNCH GOOGLE EARTH -",
                            lat_lon, guess, ctry);
                  ...
                  ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonCountry,
                            "Lat_lon '%s' MIGHT be in '%s' instead of '%s'", lat_lon, guess, ctry);
                }
              } else {
                if (! strict) {
                  /* check for Spain: Tenerife being in Spain: Canary Islands, matching Spain country only */
                  StringNCpy_0 (possible, guess, sizeof (possible));
                  ptr = StringChr (possible, ':');
                  if (ptr != NULL) {
                    *ptr = '\0';
                  }
                  if (StringICmp (possible, ctry) != 0) {
                    ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                              "Lat_lon '%s' might instead be in '%s' instead of '%s'", lat_lon, guess, ctry);
                  }
                } else {
                  ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                            "Lat_lon '%s' might instead be in '%s' instead of '%s'", lat_lon, guess, ctry);
              ...
              ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonCountry,
                        "Lat_lon '%s' does not map to '%s', but may be in '%s'", lat_lon, ctry, guess);
            }
          } else {
            bodyofwater = LookupWaterByLatLon (lat, lon);
            if (StringDoesHaveText (bodyofwater)) {
              if (CountryIsNearLatLon (ctry, lat, lon, 1.0, &distance)) {
                ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonWater,
                          "Lat_lon '%s' is nearby but not inside '%s' at distance %d km, but in water '%s'",
                          lat_lon, ctry, AdjustAndRoundDistance (distance, scale), bodyofwater);
              } else {
                ...
                if (StringDoesHaveText (guess)) {
                  if (distance < 2.0) {
                    ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonWater,
                              "Lat_lon '%s' closest country is '%s', but in water '%s'",
                              lat_lon, guess, bodyofwater);
                  } else {
                    ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonWater,
                              "Lat_lon '%s' closest country is '%s' at distance %d km, but in water '%s'",
                              lat_lon, guess, AdjustAndRoundDistance (distance, scale), bodyofwater);
                  }
                } else {
                  ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonWater,
                            "Lat_lon '%s' does not map to '%s', but in water '%s'",
                            lat_lon, ctry, bodyofwater);
                }
              }
            } else {
              if (CountryIsNearLatLon (ctry, lat, lon, 1.0, &distance)) {
                ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                          "Lat_lon '%s' is nearby but not inside '%s' at distance %d km",
                          lat_lon, ctry, AdjustAndRoundDistance (distance, scale));
              } else {
                guess = CountryClosestToLatLon (lat, lon, 5.0, &distance);
                if (StringDoesHaveText (guess)) {
                  if (distance < 2.0) {
                    ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                              "Lat_lon '%s' closest country is '%s'", lat_lon, guess);
                  } else {
                    ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonState,
                              "Lat_lon '%s' closest country is '%s' at distance %d km",
                              lat_lon, guess, AdjustAndRoundDistance (distance, scale));
                  }
                } else {
                  ValidErr (vsp, SEV_INFO, ERR_SEQ_DESCR_LatLonCountry,
                            "Lat_lon '%s' does not map to '%s'", lat_lon, ctry);
                }
              }
            }
          }
...
static void OldValidateCountryLatLon (
  ValidStructPtr vsp,
  GatherContextPtr gcp,
  CharPtr countryname,
  CharPtr lat_lon
...

{
  ...
  if (vsp != NULL && gcp != NULL && countryname != NULL && lat_lon != NULL) {
    IsCorrectLatLonFormat (lat_lon, &format_ok, &lat_in_range, &lon_in_range);
    if (! format_ok) {
      /* may have comma and then altitude, so just get lat_lon component */
      StringNCpy_0 (tmp, lat_lon, sizeof (tmp));
...
static Boolean use_older_lat_lon_validation = FALSE;
static Boolean older_lat_lon_validation_set = FALSE;
...
static void ValidateBioSource (ValidStructPtr vsp, GatherContextPtr gcp, BioSourcePtr biop, SeqFeatPtr sfp, ValNodePtr sdp)
{
  ...
  if (countryname != NULL && lat_lon != NULL) {
    csp = GetLatLonCountryData ();
    if (csp != NULL) {
#ifdef USE_OLDER_LAT_LON_VALIDATION
      use_older_lat_lon_validation = TRUE;
#else
      if (! older_lat_lon_validation_set) {
        str = (CharPtr) getenv ("USE_OLDER_LAT_LON_VALIDATION");
        if (StringDoesHaveText (str) && StringICmp (str, "TRUE") == 0) {
          use_older_lat_lon_validation = TRUE;
        }
        older_lat_lon_validation_set = TRUE;
      }
#endif
      if (use_older_lat_lon_validation) {
        PrevValidateCountryLatLon (vsp, gcp, countryname, lat_lon);
      } else {
        NewerValidateCountryLatLon (vsp, gcp, countryname, lat_lon);
      }
    } else {
      OldValidateCountryLatLon (vsp, gcp, countryname, lat_lon);
    }
  }
...
{
  Int4 str_len, suffix_len, val_len;

  str_len = StringLen (str);
  suffix_len = StringLen (suffix);
  val_len = StringLen (val);
  if (str_len < suffix_len + val_len) {
    return FALSE;
  } else if (StringNCmp (str + str_len - val_len - suffix_len, suffix, suffix_len) != 0) {
    return FALSE;
  } else if (StringCmp (str + str_len - val_len, val) != 0) {
    return FALSE;
  } else {
    return TRUE;
  }
