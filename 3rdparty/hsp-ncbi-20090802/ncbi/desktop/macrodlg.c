...
static void SummarizeMacro (DoC doc, ValNodePtr macro_list, FonT font)
{
  ...
  ColFmt[4].pixWidth = stdCharWidth;
  ColFmt[5].pixWidth = r.right - r.left - stdCharWidth - 48;
  ...
  if (macro_list == NULL) {
    ...
    for (vnp = macro_list; vnp != NULL; vnp = vnp->next) {
      ...
      tmp = (CharPtr) MemNew (sizeof (Char) * (StringLen (str) + 8));
      sprintf (tmp, "\t\t\t\tC\t%s\n", str);
      str = MemFree (str);
  ...
  if (f->macro_list == NULL) {
    ...
    if (sep_list == NULL) {
      ...
      for (vnp = sep_list; vnp != NULL; vnp = vnp->next) {
        ...
        lip->data_in_log |= ApplyMacroToSeqEntryEx (sep, f->macro_list, lip->fp);
...
static void ChangeRemoveDuplidateFeatActionButon (ButtoN b)
{
  RemoveDuplicateFeatActionDlgPtr dlg;
...
static DialoG RemoveDuplicateFeatActionDialog (GrouP h, Boolean edit, Nlm_ChangeNotifyProc change_notify, Pointer change_userdata)
{
  RemoveDuplicateFeatActionDlgPtr dlg;
  GrouP              p;
  ...
  dlg->ignore_partials = CheckBox (p, "Ignore partials", ChangeRemoveDuplidateFeatActionButon);
  SetObjectExtra (dlg->ignore_partials, dlg, NULL);
  dlg->case_sensitive = CheckBox (p, "Case sensitive", ChangeRemoveDuplidateFeatActionButon);
  SetObjectExtra (dlg->case_sensitive, dlg, NULL);
  SetStatus (dlg->case_sensitive, TRUE);

  dlg->remove_proteins = CheckBox (p, "Remove proteins", ChangeRemoveDuplidateFeatActionButon);
...
static void RunMacroInEditor (ButtoN b)
{
  ...
  if (sep_list == NULL) {
    ...
    for (vnp = sep_list; vnp != NULL; vnp = vnp->next) {
      sep = vnp->data.ptrvalue;
      entityID = ObjMgrGetEntityIDForChoice(sep);
      lip->data_in_log |= ApplyMacroToSeqEntryEx (sep, action, lip->fp);
      ObjMgrSetDirtyFlag (entityID, TRUE);
      ObjMgrSendMsg (OM_MSG_UPDATE, entityID, 0, 0);
    }
    sep_list = ValNodeFree (sep_list);
...
static void SingleMacroAcceptButton (ButtoN b)
{
  ...
  num_fields = 0;
  num_features = 0;
  ApplyMacroToSeqEntry (sep, new_action);
...
static void RunSelectedMacro (ButtoN b)
{
  ...
  if (sep_list == NULL) {
    ...
    for (vnp = sep_list; vnp != NULL; vnp = vnp->next) {
      sep = vnp->data.ptrvalue;
      entityID = ObjMgrGetEntityIDForChoice(sep);
      lip->data_in_log |= ApplyMacroToSeqEntryEx (sep, tmp_list, lip->fp);
      ObjMgrSetDirtyFlag (entityID, TRUE);
      ObjMgrSendMsg (OM_MSG_UPDATE, entityID, 0, 0);
    }
    sep_list = ValNodeFree (sep_list);
