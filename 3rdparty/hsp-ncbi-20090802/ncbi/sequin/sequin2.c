...
NLM_EXTERN void SetSequencesForSubmissionForm (WindoW w, SeqEntryPtr sep_list, Int4 page)
{
  ...
  SeqEntryPtrToSourceTab (sqfp);
  if (page > 0 && page < 4) {
    ChangeSequencesPage (sqfp, page, 0);
    SetValue (sqfp->tbs, page);
  }
}
...
static void ChooseCategoriesByMacroStringConstraint (ValNodePtr value_list, StringConstraintPtr scp, Boolean do_choose)
{
  ClickableItemPtr cip;
    ...
    if (cip != NULL) {
      if (cip->item_list != NULL 
        ...
        cip->chosen = do_choose;
          ...
          ChooseCategoriesByMacroStringConstraint (cip->subcategories, scp, do_choose);
      }
    }
...
static void SelectSequenceIDsForRemoval (ButtoN b)
{
  ...
  ChooseCategoriesByMacroStringConstraint (rp->seq_list, scp, TRUE);
...
extern void RemoveSequencesFromRecord (IteM i)
{
  ...
  k = HiddenGroup (h, 2, 0, NULL);
  rp->constraint_dlg = StringConstraintDialog (k, "Mark sequences where sequence ID", TRUE, NULL, NULL);
