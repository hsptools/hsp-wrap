...
case GNU/Linux:
	...
	default:
		...
		if ("$?LINUX_MODE" == 1) then
			...
			if ("$LINUX_MODE" == "pgicc") then
				set platform=${platform}_pgicc
			endif
			if ("$LINUX_MODE" == "mic") then
				set platform=${platform}_mic
			endif
		endif
		breaksw
	endsw
...
# PG!! Tweak options
set NCBI_CFLAGS1 = "$NCBI_CFLAGS1 -DSTDIOWRAP_AUTO -include ${HSP_TOP}/lib/stdiowrap/stdiowrap/stdiowrap.h"
set NCBI_LDFLAGS1 = "$NCBI_LDFLAGS1 -L${HSP_TOP}/build/lib/stdiowrap"
set NCBI_OTHERLIBS = "-Wl,-Bstatic -lstdiowrap $NCBI_OTHERLIBS -Wl,-Bdynamic -lrt" 
