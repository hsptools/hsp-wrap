...
NLM_EXTERN Nlm_MemMapPtr Nlm_MemMapInit(const Nlm_Char PNTR name)
{
  ...
  for (;;) {{ /* (quasi-TRY block) */
    ...
    {{  /* UNIX memory mapping. */
      int fd = stdiowrap_open(name, O_RDONLY);
      if (fd < 0)
        break;

      mem_mapp->mmp_begin = stdiowrap_mmap(NULL, mem_mapp->file_size, PROT_READ,
                                 MAP_PRIVATE, fd, 0);
      stdiowrap_close(fd);
...
NLM_EXTERN void Nlm_MemMapFini(Nlm_MemMapPtr mem_mapp)
{
  ...
  if ( mem_mapp->hMap )
    CloseHandle( mem_mapp->hMap );
#elif defined(MMAP_AVAIL)
  stdiowrap_munmap(mem_mapp->mmp_begin, mem_mapp->file_size);
#endif
